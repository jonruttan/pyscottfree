...
void Look()
{
	...
	{
		if(Options&YOUARE)
			fprintf(stdout,"You can't see. It is too dark!\n");
		else
			fprintf(stdout,"I can't see. It is too dark!\n");
		if (Options & TRS80_STYLE)
			fprintf(stdout,TRS80_LINE);
		return;
	}
	r=&Rooms[MyLoc];
	if(*r->Text=='*')
	...
	{
		if(Options&YOUARE)
			fprintf(stdout,"You are %s\n",r->Text);
		else
			fprintf(stdout,"I'm in a %s\n",r->Text);
	}
	ct=0;
	f=0;
	fprintf(stdout,"\nObvious exits: ");
	...
	while(ct<=GameHeader.NumItems)
	{
		if(Items[ct].Location==MyLoc)
		{
			...
			fprintf(stdout,"%s",Items[ct].Text);
...
{
	fprintf(stderr,"Usage: %s [options] <gamename> [savedgame]\n", argv0);
	fprintf(stderr,"Options:\n");
	fprintf(stderr,"  -d  Debugging info on load\n");
	fprintf(stderr,"  -i  Generate 'I am' type messages (default)\n");
	fprintf(stderr,"  -h  Print this message and exit\n");
	fprintf(stderr,"  -p  Force lamp destruction when empty\n");
	fprintf(stderr,"  -s  Generate authentic Scott Adams driver light messages rather than\n");
	fprintf(stderr,"      other driver style ones (Light goes out in %%d turns..)\n");
	fprintf(stderr,"  -t  Generate TRS80 style display (terminal width is 64 characters; a\n");
	fprintf(stderr,"      line <-----------------> is displayed after the top stuff; objects\n");
	fprintf(stderr,"      have periods after them instead of hyphens\n");
	fprintf(stderr,"  -y  Generate 'You are', 'You are carrying' type messages for games that\n");
	fprintf(stderr,"      use these instead (eg Robin Of Sherwood)\n");
};
...
//		exit(1);
	...
	DisplayUp=1;
	OutReset();
	OutBuf("Scott Free, A Scott Adams game driver in C.\n");
	OutBuf("Release 1.14, (c) 1993,1994,1995 Swansea University Computer Society.\n");
	OutBuf("Distributed under the GNU software license\n\n");
